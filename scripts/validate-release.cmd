@echo off
rem ECMP Governor release validation.
rem
...
if !FAILED!==1 ( echo RELEASE CONFIGURE FAILED & exit /b 1 )

echo == Release build ==
cmake --build "%WORK_DIR%\release"
if errorlevel 1 set FAILED=1
if !FAILED!==1 ( echo RELEASE BUILD FAILED & exit /b 1 )

echo == Release test ==
ctest --test-dir "%WORK_DIR%\release" --output-on-failure
if errorlevel 1 set FAILED=1
if !FAILED!==1 ( echo RELEASE TESTS FAILED & exit /b 1 )

echo == Debug configure ==
cmake -S "%SOURCE_DIR%" -B "%WORK_DIR%\debug" -G Ninja -DCMAKE_BUILD_TYPE=Debug
if errorlevel 1 set FAILED=1
if !FAILED!==1 ( echo DEBUG CONFIGURE FAILED & exit /b 1 )

echo == Debug build ==
cmake --build "%WORK_DIR%\debug"
if errorlevel 1 set FAILED=1
if !FAILED!==1 ( echo DEBUG BUILD FAILED & exit /b 1 )

echo == Debug test ==
ctest --test-dir "%WORK_DIR%\debug" --output-on-failure
if errorlevel 1 set FAILED=1
if !FAILED!==1 ( echo DEBUG TESTS FAILED & exit /b 1 )
...
echo == Benchmark ==
"%WORK_DIR%\release\bench\ecmp_benchmark.exe"
if errorlevel 1 ( echo BENCHMARK FAILED & exit /b 1 )

echo == CLI version ==
"%WORK_DIR%\release\ecmp_cli.exe" version
if errorlevel 1 ( echo CLI VERSION FAILED & exit /b 1 )

echo == CLI store inspect ==
"%WORK_DIR%\release\ecmp_cli.exe" store inspect --store "%WORK_DIR%\cli-inspect.store" 2>&1 | findstr /C:"ECMP_CLI_ERROR" >nul
